...
class PlanningDataAnalyser {
  static async run() {
    ...
    Logger.info(
      `Configuration: Timeout=${CONFIG.http.timeout}ms, Retries=${CONFIG.http.retryAttempts}, Rate Limit=${CONFIG.http.rateLimit.maxRequests}/${CONFIG.http.rateLimit.perMilliseconds}ms`
    )
    ...
    try {
      ...
      const planningApplicationsJson = path.join(
        CONFIG.rawDir,
        'planning-applications.json'
      )
      const localAuthoritiesJson = path.join(
        CONFIG.rawDir,
        'local-authorities.json'
      )
      const localPlanningAuthoritiesJson = path.join(
        CONFIG.rawDir,
      ...
      if (!(await FileManager.fileExists(planningApplicationsJson))) {
        Logger.info(
          'Planning applications data not found, extracting from API...'
        )
        planningApplications = await DataExtractor.extractPlanningApplications()
        await FileManager.saveJson(
          planningApplications,
          planningApplicationsJson
        )
        Logger.success(
          `Saved ${planningApplications.length} planning applications to cache`
        )
      } else {
        Logger.info('Loading planning applications from cache...')
        planningApplications = await FileManager.loadJson(
          planningApplicationsJson
        )
        Logger.success(
          `Loaded ${planningApplications.length} planning applications from cache`
        )
      }
      ...
      if (!(await FileManager.fileExists(localAuthoritiesJson))) {
        ...
        Logger.success(
          `Saved ${localAuthorities.length} local authorities to cache`
        )
      } else {
        Logger.info('Loading local authorities from cache...')
        localAuthorities = await FileManager.loadJson(localAuthoritiesJson)
        Logger.success(
          `Loaded ${localAuthorities.length} local authorities from cache`
        )
      }
      ...
      if (!(await FileManager.fileExists(localPlanningAuthoritiesJson))) {
        Logger.info(
          'Local planning authorities data not found, extracting from API...'
        )
        localPlanningAuthorities =
          await DataExtractor.extractLocalPlanningAuthorities()
        await FileManager.saveJson(
          localPlanningAuthorities,
          localPlanningAuthoritiesJson
        )
        Logger.success(
          `Saved ${localPlanningAuthorities.length} local planning authorities to cache`
        )
      } else {
        Logger.info('Loading local planning authorities from cache...')
        localPlanningAuthorities = await FileManager.loadJson(
          localPlanningAuthoritiesJson
        )
        Logger.success(
          `Loaded ${localPlanningAuthorities.length} local planning authorities from cache`
        )
      }
      ...
      Logger.success(
        `Enhanced ${enhancedApplications.length} applications and saved to cache`
      )
      ...
      const standardisedApplications =
        DataStandardiser.standardiseDevelopmentCategories(enhancedApplications)

      // Populate local authority names
      const applicationsWithAuthorityNames =
        DataStandardiser.populateLocalAuthorityNames(
          standardisedApplications,
          localAuthorities
        )

      // Populate local planning authority names
      const fullyEnhancedApplications =
        DataStandardiser.populateLocalPlanningAuthorityNames(
      ...
      const standardisedDataPath = path.join(
        CONFIG.enhancedDir,
        'enhanced-planning-applications-standardised.json'
      )
      DataStandardiser.saveStandardisedData(
        fullyEnhancedApplications,
        standardisedDataPath
      )

      // Step 4: Statistical analysis
      Logger.info('=== Step 4: Statistical Analysis ===')
      const analysis = StatisticalAnalyser.analyzePlanningData(
        fullyEnhancedApplications
      )
      ...
      csvResults.forEach((result) => {
        if (result.success) {
          Logger.success(
            `✓ ${result.file} generated successfully (${result.size} ${result.size.includes('MB') ? 'MB' : 'KB'})`
          )
          successfulCsvFiles++
        } else {
      ...
      Logger.info(
        `CSV Generation: ${successfulCsvFiles}/${csvResults.length} files created successfully`
      )
      ...
      Logger.info(
        `Rate Limit: ${extractionStats.config.rateLimit.maxRequests} requests per ${extractionStats.config.rateLimit.perMilliseconds}ms`
      )

      // Report standardisation statistics
      const categoryStats = DataStandardiser.getDevelopmentCategoryStats(
        fullyEnhancedApplications
      )
  ...
  static async regenerateFromExistingData(enhancedDataPath = null) {
    ...
    try {
      ...
      const dataPath =
        enhancedDataPath ||
        path.join(CONFIG.enhancedDir, 'enhanced-planning-applications.json')
      ...
      Logger.success(
        `Loaded ${enhancedApplications.length} enhanced applications`
      )
      ...
      if (await FileManager.fileExists(localPlanningAuthoritiesJson)) {
        ...
        Logger.info(
          `Loaded ${localPlanningAuthorities.length} local planning authorities`
        )
      }
