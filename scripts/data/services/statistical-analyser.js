...
class StatisticalAnalyser {
  static analyzePlanningData(enhancedApplications) {
    ...
    const analysis = {
      summary: {
        ...
        applicationsWithHouseCount: enhancedApplications.filter(
          (app) => app.houseCount > 0
        ).length,
        applicationsWithCommercialCount: enhancedApplications.filter(
          (app) => app.commercialBuildingCount > 0
        ).length,
        applicationsWithInfrastructureCount: enhancedApplications.filter(
          (app) => app.infrastructureBuildCount > 0
        ).length,
        totalHouses: enhancedApplications.reduce(
          (sum, app) => sum + (app.houseCount || 0),
          0
        ),
        totalCommercialBuildings: enhancedApplications.reduce(
          (sum, app) => sum + (app.commercialBuildingCount || 0),
          0
        ),
        totalInfrastructureProjects: enhancedApplications.reduce(
          (sum, app) => sum + (app.infrastructureBuildCount || 0),
          0
        )
      },
      byDevelopmentCategory:
        this.analyzeByDevelopmentCategory(enhancedApplications),
      houseBuildStatistics:
        this.analyzeHouseBuildStatistics(enhancedApplications),
      commercialBuildStatistics:
        this.analyzeCommercialBuildStatistics(enhancedApplications),
      infrastructureBuildStatistics:
        this.analyzeInfrastructureBuildStatistics(enhancedApplications),
      houseCountDistribution:
        this.analyzeHouseCountDistribution(enhancedApplications),
      commercialCountDistribution:
        this.analyzeCommercialCountDistribution(enhancedApplications),
      infrastructureCountDistribution:
        this.analyzeInfrastructureCountDistribution(enhancedApplications)
    }
  ...
  static analyzeByDevelopmentCategory(applications) {
    ...
    applications.forEach((app) => {
      ...
      categories[category].totalCommercialBuildings +=
        app.commercialBuildingCount || 0
      categories[category].totalInfrastructureProjects +=
        app.infrastructureBuildCount || 0
    })

    // Calculate averages
    Object.values(categories).forEach((cat) => {
      cat.averageHouses = cat.count > 0 ? cat.totalHouses / cat.count : 0
      cat.averageCommercialBuildings =
        cat.count > 0 ? cat.totalCommercialBuildings / cat.count : 0
      cat.averageInfrastructureProjects =
        cat.count > 0 ? cat.totalInfrastructureProjects / cat.count : 0
    })
  ...
  static analyzeHouseBuildStatistics(applications) {
    const houseBuildApps = applications.filter(
      (app) => app.developmentCategory === 'house_build' && app.houseCount > 0
    )
    ...
    return {
      ...
      averageHouses:
        houseCounts.reduce((sum, count) => sum + count, 0) / houseCounts.length,
      totalHouses: houseCounts.reduce((sum, count) => sum + count, 0)
    }
  }

  static analyzeCommercialBuildStatistics(applications) {
    const commercialBuildApps = applications.filter(
      (app) =>
        app.developmentCategory === 'commercial_build' &&
        app.commercialBuildingCount > 0
    )
    ...
    const commercialCounts = commercialBuildApps.map(
      (app) => app.commercialBuildingCount
    )
    ...
    return {
      ...
      averageCommercialBuildings:
        commercialCounts.reduce((sum, count) => sum + count, 0) /
        commercialCounts.length,
      totalCommercialBuildings: commercialCounts.reduce(
        (sum, count) => sum + count,
        0
      )
    }
  ...
  static analyzeInfrastructureBuildStatistics(applications) {
    const infrastructureBuildApps = applications.filter(
      (app) =>
        app.developmentCategory === 'infrastructure_build' &&
        app.infrastructureBuildCount > 0
    )
    ...
    const infrastructureCounts = infrastructureBuildApps.map(
      (app) => app.infrastructureBuildCount
    )
    ...
    return {
      ...
      averageInfrastructureProjects:
        infrastructureCounts.reduce((sum, count) => sum + count, 0) /
        infrastructureCounts.length,
      totalInfrastructureProjects: infrastructureCounts.reduce(
        (sum, count) => sum + count,
        0
      )
    }
  ...
  static generateAnalysisCSV(analysis, outputPath) {
    try {
      ...
      const categoryRows = this.convertDevelopmentCategoriesToCSV(
        analysis.byDevelopmentCategory
      )
  ...
  static convertSummaryToCSV(summary) {
    return {
      ...
      averageHouses: (summary.totalHouses / summary.totalApplications).toFixed(
        2
      ),
      averageCommercialBuildings: (
        summary.totalCommercialBuildings / summary.totalApplications
      ).toFixed(2),
      averageInfrastructureProjects: (
        summary.totalInfrastructureProjects / summary.totalApplications
      ).toFixed(2)
    }
  }
}
