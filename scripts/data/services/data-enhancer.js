...
class DataEnhancer {
    static async enhancePlanningData(planningApplications, localAuthorities, localPlanningAuthorities) {
        ...
        const localPlanningAuthorityMap = this.createLocalPlanningAuthorityMap(localPlanningAuthorities);

        // Extract all descriptions for batch LLM processing
        const descriptions = planningApplications.map(app => app.description || '');
        ...
        try {
            ...
            for (let i = 0; i < planningApplications.length; i++) {
                ...
                Logger.info(`Enhancing application ${i + 1}/${planningApplications.length}`);
                ...
                try {
                    ...
                    Logger.error(`Failed to enhance application ${application.entity || i}`, error);
            ...
            Logger.error('Batch LLM analysis failed, falling back to individual processing', error);

            // Fallback to individual processing if batch fails
            for (let i = 0; i < planningApplications.length; i++) {
                const application = planningApplications[i];
                Logger.info(`Enhancing application ${i + 1}/${planningApplications.length} (individual mode)`);
        ...
        Logger.success(`Enhanced ${enhancedApplications.length} planning applications`);
        return enhancedApplications;
    }

    static async enhanceSingleApplication(application, llmAnalysis, localAuthorityMap, localPlanningAuthorityMap) {
        ...
        const organizationId = application['organisation-entity'] || application.organization_id;
        const localAuthorityName = localAuthorityMap.get(organizationId) || 'Unknown';
        ...
        const localPlanningAuthorityName = localPlanningAuthorityMap.get(planningAuthorityRef) || 'Unknown';
