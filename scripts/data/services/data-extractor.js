...
class DataExtractor {
  ...
  static async extractPlanningApplications() {
    ...
    while (hasMoreData && failedRequests < maxFailedRequests) {
      ...
      try {
        Logger.info(
          `Fetching planning applications (offset: ${offset}, total retrieved: ${totalRetrieved})...`
        )
        ...
        if (response && response.entities && Array.isArray(response.entities)) {
          ...
          Logger.info(
            `Retrieved ${newApplications.length} applications in ${requestTime}ms`
          )

          if (newApplications.length < CONFIG.chunkSize) {
            hasMoreData = false
            Logger.info(
              'Reached end of data (received fewer records than chunk size)'
            )
          } else {
        ...
        failedRequests++
        Logger.error(
          `Failed to extract planning applications at offset ${offset}: ${error.message}`
        )

        if (failedRequests >= maxFailedRequests) {
          Logger.error(
            `Stopping extraction after ${maxFailedRequests} consecutive failures`
          )
          break
        }
    ...
    Logger.success(
      `Extracted ${allApplications.length} planning applications (${failedRequests} failed requests)`
    )
  ...
  static async extractLocalAuthorities() {
    ...
    try {
      ...
      if (response && response.entities && Array.isArray(response.entities)) {
        Logger.success(
          `Extracted ${response.entities.length} local authorities in ${requestTime}ms`
        )
  ...
  static async extractLocalPlanningAuthorities() {
    ...
    try {
      ...
      if (response && response.entities && Array.isArray(response.entities)) {
        Logger.success(
          `Extracted ${response.entities.length} local planning authorities in ${requestTime}ms`
        )
        return response.entities
      } else {
