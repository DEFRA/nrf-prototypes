...
class CSVGenerator {
  ...
  static async generateCSVFiles(
    ...
    const enhancedPlanningCSVPath = path.join(
      process.cwd(),
      'data',
      'output',
      'enhanced-planning-data.csv'
    )
    const enhancedPlanningResult = this.generateEnhancedPlanningCSV(
      enhancedApplications,
      enhancedPlanningCSVPath
    )
  ...
  static generateEnhancedPlanningCSV(planningApps, outputPath) {
    try {
      ...
      Logger.success(
        `Enhanced planning CSV generated: ${outputPath} (${fileSize} MB)`
      )
  ...
  static generateLocalPlanningAuthoritiesCSV(authorities, outputPath) {
    try {
      ...
      const fileSize = (csvContent.length / 1024).toFixed(2)
      Logger.success(
        `Authorities CSV generated: ${outputPath} (${fileSize} KB)`
      )
  ...
  static convertAuthoritiesToCSV(authorities) {
    ...
    const rows = authorities.map((authority) => {
      return [authority.reference || '', authority.name || '']
        .map((field) => `"${field}"`)
        .join(',')
    })
