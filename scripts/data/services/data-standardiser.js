...
class DataStandardiser {
    ...
    static CATEGORY_MAPPING = {
        ...
        'mixed-use development comprising residential units and commercial floorspace': 'mixed_use',
        'mixed-use development comprising residential units and commercial space etc.': 'mixed_use',
    ...
    static standardiseDevelopmentCategories(planningApps) {
        ...
        const standardisedApps = planningApps.map(app => {
            ...
            if (originalCategory !== standardisedCategory) {
                updatedCount++;
                if (updatedCount <= 10) {
                    Logger.info(`  ${app.entity}: "${originalCategory}" → "${standardisedCategory}"`);
                }
            }
    ...
    static populateLocalAuthorityNames(planningApps, localAuthorities) {
        ...
        Logger.info(`Created lookup map with ${Object.keys(authorityLookup).length} local authorities`);
        ...
        Logger.success(`Updated ${updatedCount} planning applications with local authority names`);
        Logger.info(`Could not find local authority for ${notFoundCount} applications`);
     ...
     */
    static populateLocalPlanningAuthorityNames(planningApps, localAuthorities, localPlanningAuthorities) {
        Logger.info('Populating local planning authority names...');

        // Create lookup maps
        const authorityToPlanningAuthorityLookup = {};
        localAuthorities.forEach(authority => {
            if (authority['organisation-entity'] && authority['local-planning-authority']) {
                authorityToPlanningAuthorityLookup[authority['organisation-entity']] = authority['local-planning-authority'];
            }
        });
        ...
        Logger.info(`Created lookup maps: ${Object.keys(authorityToPlanningAuthorityLookup).length} authority mappings, ${Object.keys(planningAuthorityLookup).length} planning authorities`);
        ...
        const updatedApps = planningApps.map(app => {
            ...
            if (orgEntity && authorityToPlanningAuthorityLookup[orgEntity]) {
                const planningAuthorityCode = authorityToPlanningAuthorityLookup[orgEntity];

                if (planningAuthorityLookup[planningAuthorityCode]) {
                    app.localPlanningAuthorityName = planningAuthorityLookup[planningAuthorityCode];
                    updatedCount++;
                } else {
        ...
        Logger.success(`Updated ${updatedCount} planning applications with local planning authority names`);
        Logger.info(`Could not find local planning authority for ${notFoundCount} applications`);
