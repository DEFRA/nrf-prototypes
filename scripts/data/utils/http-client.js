...
class HttpClient {
    constructor() {
        ...
        axiosRetry(this.client, {
            ...
            retryDelay: (retryCount) => {
                const delay = CONFIG.http.retryDelay * Math.pow(CONFIG.http.retryDelayMultiplier, retryCount - 1);
            ...
            onRetry: (retryCount, error, requestConfig) => {
                Logger.warn(`Request failed, retrying (${retryCount}/${CONFIG.http.retryAttempts}): ${error.message}`);
            }
        });
        ...
        this.client.interceptors.response.use(
            ...
            (error) => {
                if (error.response) {
                    Logger.error(`Response error from ${error.config.url}: ${error.response.status} - ${error.response.statusText}`);
     ...
     * @returns {Promise<Object>} - The response data
     */
    async makeRequestWithRetry(url, options = {}, retries = CONFIG.http.retryAttempts) {
