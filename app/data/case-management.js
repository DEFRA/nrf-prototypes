...
function getFilteredApplications(filters = {}) {
  ...
  if (filters.dateFrom) {
    ...
    applications = applications.filter(
      (app) => new Date(app.createdAt) >= fromDate
    )
  }
  ...
  if (filters.dateTo) {
    ...
    applications = applications.filter(
      (app) => new Date(app.createdAt) <= toDate
    )
  }
...
function updateApplicationWithAudit(id, updates, userId, userRole, reason) {
  ...
  if (updatedApplication.quote && application.quote) {
    const quoteChanged =
      updatedApplication.quote.total !== application.quote.total
    if (quoteChanged) {
