...
;(function () {
  ...
  function createStatsPanel() {
    ...
    panel.innerHTML = `
      <div class="map-stats-content">
        ...
        <div class="govuk-!-margin-top-4" style="text-align: center;">
          <button class="govuk-button govuk-!-margin-bottom-0" type="submit" id="save-and-continue-btn" disabled style="width: 100%;">
            Save and continue
          </button>
        </div>
      </div>
  ...
  function showIntersectionsLoading() {
    ...
    const loadingHtml = `
      <div class="govuk-!-text-align-centre govuk-!-padding-2 govuk-!-text-colour-secondary">
        <p class="govuk-body govuk-!-margin-0">Loading EDP information...</p>
      </div>
    `
  ...
  function updateStatsDisplay(stats) {
    ...
    // NOTE: Panel visibility is managed by init.js event handlers
    // Don't show/hide panel here - it causes issues during editing
  }
  ...
  function handlePolygonComplete(feature) {
    ...
    // NOTE: Panel visibility is managed by init.js draw:done handler
    // Don't show/hide panel here
  }
  ...
  function handlePolygonEdit(feature) {
    // Update stats only during editing, don't call handlePolygonComplete
    // which would show the boundary information panel
    if (!feature) return
    ...
    // Only update the stats display, don't show intersections during editing
    updateStatsDisplay(currentStats)
  }
  ...
  function handleEditStart() {
    ...
    // Hide the entire stats panel during editing
  ...
  function init(mapInstance, drawInstance) {
    ...
    const mapContainer = document.getElementById('stats-panel-slot') || document.getElementById('map')
    if (!mapContainer) {
