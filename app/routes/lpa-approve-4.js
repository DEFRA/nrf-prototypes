//
// LPA Approval Journey Routes - LPA approve developer request to pay NRF (v4)
// Includes Approve and Reject paths
//

const govukPrototypeKit = require('govuk-prototype-kit')
const router = govukPrototypeKit.requests.setupRouter()
const { ROUTES, TEMPLATES } = require('../config/lpa-approve-4/routes')
...
// Approve these details (Approve → lpa-details, Reject → lpa-details-2)
...
// Enter your details (approve path)
...
// Enter your details (reject path)
router.get(ROUTES.LPA_DETAILS_2, (req, res) => {
  const data = req.session.data || {}
  res.render(TEMPLATES.LPA_DETAILS_2, {
    data,
    backLink: ROUTES.APPROVE_DETAILS
  })
})

router.post(ROUTES.LPA_DETAILS_2, (req, res) => {
  const fullName = req.body['full-name']?.trim()
  const email = req.body.email?.trim()
  const errors = []
  const errorsByField = {}
  ...
  if (errors.length > 0) {
    return res.render(TEMPLATES.LPA_DETAILS_2, {
      errors,
      errorsByField,
      data: req.session.data || {},
      backLink: ROUTES.APPROVE_DETAILS
  ...
  req.session.save((err) => {
    ...
    res.redirect(ROUTES.REASON_FOR_REJECTING)
  })
})
