...
- **Data Display**:
  ...
  - **Casing in running sentences**: When a stored option value (e.g. a radio value like `data.planningType` → "Full") is inlined into running sentence text such as "has full planning permission", apply the Nunjucks `| lower` filter so it reads naturally: `{{ data.planningType | lower }}`. Do **not** lowercase values that are proper nouns or place names (e.g. `data.redlineBoundaryPolygon.intersectingCatchment` → "Thames Valley EDP"), and do **not** lowercase the same value where it appears as a standalone summary-list value (e.g. on a check your answers page it should keep its stored casing, "Full").
