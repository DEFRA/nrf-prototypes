...
- **Content-driven journeys reload live**: `nrf-quote-7` and `nrf-request-to-use-1` are built from `content/<id>` (markdown copy + `journey.yaml`) by `app/lib/journey-engine/`. Every `content/<id>/journey.yaml` is mounted automatically by `app/routes.js` (no route file, no registry entry; a new folder needs one restart to mount). The engine re-reads content per request and mounts one dispatcher under the journey's basePath, so editing copy, rules, or adding/removing pages needs no restart; `app/lib/journey-engine/watch.js` watches `content/` in development and pokes browser-sync so the open tab reloads itself. Only `basePath` changes and hook code (`app/lib/<journey>/hooks.js`) still need a restart. Pages listed with `shared: true` in `journey.yaml` take their copy from `content/shared/pages/<id>.md` instead of the journey's own folder (the common start page and "What would you like to do?" funnel); `shared: one-login | government-gateway | defra-id` reads the mock identity-provider pages from `content/shared/<folder>/` (sign in, create an account, Defra ID registration; the flow between them stays in each journey's `journey.yaml`, see `nrf-request-to-use-1`). A `goto` beginning with `/` exits to another journey and appears as a dashed node on the tools page. A sibling `pages/<id>~<name>.md` is a **copy variant** of a page (alternative wording only; the loader refuses changed option values, fields or type): shown with `?copy=<name>` for the session or `?_copy=<name>` once, as an orange-topped card on the wall with a "Compare side by side" view at `/tools/journeys/<id>/compare/<page>`, exported as `<page>~<name>.jpg`, and never in a frozen handoff copy. See `content/README.md` "Trying out variations of a page".
...
- **Design handoff is per page**: `handoff: YYYY-MM-DD` on a page entry in `content/<id>/journey.yaml` marks it ready for dev. `app/lib/journey-engine/history.js` asks git whether the copy changed since (the tools page and homepage card show turquoise/yellow tags); the deployed image has no `.git`, so the Publish workflow runs `npm run handoff:manifest` to bake `app/data/content-history.json` and `scripts/handoff-tags.js` to tag each new handoff `handoff/<journey>/<date>`. Every committed handoff also has a frozen copy at `/handoffs/<journey>/<date>/<page>`: `app/routes/handoffs.js` serves the journey from a `git archive` snapshot of the handoff commit (`app/lib/journey-engine/snapshots.js`, extracted on demand into `.tmp/handoffs/` locally and baked into `app/data/handoffs/` by `npm run handoff:snapshots` in the Publish workflow). See `content/README.md` "Handing pages to development".
- **Tests must not hard-code journey copy.** Specs for the content-driven journeys read headings, errors, option labels, field labels, buttons and links from `content/` via `copyOf(id)` in `tests/e2e/helpers/journey.js` (`answer`, `fillAnswer`, `fillField`, `submit`, `act`, `followLink`, `changeLink`, `expectHeading`, `expectError`). Literals are fine for design-system chrome (`Continue`, `Back`) and fixture data. `tests/e2e/test-conventions.spec.js` fails on literal heading/error assertions and `getByLabel(/regex/)`. Options the journey branches on carry an explicit `value:`; the loader refuses a `journey.yaml` rule that compares an answer with a value no option can store. See `tests/README.md`.
...
- **LPA journey and the `staff` layout** (`content/lpa-manage-1`, "Manage commitments to the nature restoration levy"): officers sign in with the shared One Login pages, retrieve a developer's commitment by NRL reference, add the planning reference and manage records (dashboard, searchable table, record page with audit timeline and a `<dialog>` Change status popover posting to the `change-status` radios page). `layout: staff` (set journey-wide with `layout:` in journey.yaml) renders `includes/staff-header.html` (crown, service name, account toggle with `header.organisation`, Menu; `staff-header.js`) and widens `.govuk-width-container` to 1440px (`_staff.scss`). Canned commitments/records/statuses live in `content/lpa-manage-1/records.yaml` (data, re-read on change by `app/lib/lpa-manage-1/hooks.js`); records the officer adds or changes are kept in `data.lpaRecords`. The dashboard, table and record pages are `type: custom` templates in `app/views/lpa-manage-1/` whose words come from each page's `text:` frontmatter. Shared summary rows: `- include: <name>` in `rows:` splices `partials/<name>.md`. `:::map key os` asks for the OS basemap; the map assets now load from `journey-engine/_page.html` for any page with a map.
- **Participant details box** (user research aid): `app/routes/research.js` + `app/views/includes/research-participant.html`, opened from the footer link on every page or `?participant` on any URL. It stores `data.researchParticipant` (`firstName`, `lastName`, `organisation`, `fullName`); `app/lib/nrf-request-to-use-1/hooks.js` (`withParticipant`, `participantOf`) uses those in place of `MOCK_NAME` and the fixture businesses and prefills the name pages. The dialog waits in a `<template>` until opened so its "First name"/"Last name" inputs never sit beside the journey's own (which breaks `getByLabel` in tests). The kit's nunjucks has no `{% set %}…{% endset %}` block and macros cannot see the template context.
