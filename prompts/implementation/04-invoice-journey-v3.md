...
Hint text: Enter the email address for the person that is responsible for your planning application. We will also send a copy to: [dynamically displays data.email or defaults to 'email@email.co.uk']
...
| Your details | [show full name, business name if provided, and address] |
...
[Bulleted list dynamically displays selected levies from data.leviesSelected, or defaults to showing both levies if none selected]
